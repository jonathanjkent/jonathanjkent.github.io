---
title: "Wednesdays - BCN Human Mobility"
...
flows.wed <- d %>% filter(dia == "Wednesday" & codigo_residencia != codigo_destino) %>% group_by(codigo_residencia,codigo_destino) %>% summarise(count = mean(flujo)) %>% rename(origin = codigo_residencia, dest = codigo_destino)
