...
```{r eng}
d <- read_rds("~/GoogleDrive/Futbol/leads_data_eng1.rds") %>% filter(match != "/en/matches/") %>% mutate(minute = as.numeric(gsub("\\D","",gsub("\\+.*","",minute))), home.score = as.numeric(home.score), away.score = as.numeric(away.score))
...
print(ggplot() + geom_rect(aes(xmin = .17, xmax = .58, ymin = .09, ymax = .17), alpha = 0.1) + geom_line(data=d,aes(pct_left,pred,color=league),linewidth = 1.2) + theme_minimal() + labs(y = "Prob. of Losing Lead", x = "Time Elapsed (%) When 2 Score Lead Reached", color = NULL) + theme(legend.position="bottom"))
```
...
## Is there an equivalent to a 2 goal lead in other sports?

```{r nba1}
nba <- bind_rows(read_rds("~/GoogleDrive/Futbol/leads_data_nba_pt1.rds"),read_rds("~/GoogleDrive/Futbol/leads_data_nba_pt2.rds"))
epl <- read_rds("~/GoogleDrive/Futbol/leads_data_eng1.rds") %>% filter(match != "/en/matches/") %>% mutate(minute = as.numeric(gsub("\\D","",gsub("\\+.*","",minute))), home.score = as.numeric(home.score), away.score = as.numeric(away.score)) %>% select(-contains("final"))
```

```{r nba2, echo=T}
print(epl %>% bind_rows(tibble(match=unique(epl$match),home.score=0,away.score=0,minute=0)) %>% bind_rows(expand_grid(match=unique(epl$match),minute=1:90)) %>% arrange(match,minute) %>% fill(everything()) %>% mutate(margin = abs(home.score-away.score)) %>% group_by(margin) %>% count() %>% ungroup() %>% mutate(pct = n/sum(n), cum_pct = cumsum(pct)))

print(nba %>% mutate(margin = abs(home_score-away_score)) %>% group_by(margin) %>% count() %>% ungroup() %>% mutate(pct = n/sum(n), cum_pct = cumsum(pct)) %>% filter(cum_pct > .75))
```

In the Premier League, 83.7% of the time the score margin is less than 2. In the NBA, 84% of the time the margin is less than 14. So, a 14+ point lead in the NBA is similar in frequency to a 2 goal lead. By the same logic, two touchdown leads are indeed equivalent to two goal leads. If we treat 14 point NBA leads as equivalent, then:

```{r nba3}
d <- nba %>% filter(minute<=48)
...
d <- bind_rows(preds_eng,preds_ger,preds_nhl,preds_nfl,preds_nba) %>% filter(str_sub(lead,1,1) == 2) %>% mutate(pct_left = ifelse(league %in% c("eng","ger"),minute/90,ifelse(league == "nba",minute/48,minute/60)), league = case_when(league == "eng" ~ "Soccer (EPL)", league == "ger" ~ "Soccer (Bundesliga)", league == "nhl" ~ "Hockey (NHL)", league == "nfl" ~ "Am. Football (NFL)", league == "nba" ~ "Basketball (NBA)"))

ggplot() + geom_line(data=d,aes(pct_left,pred,color=league),linewidth = 1.2) + theme_minimal() + labs(y = "Prob. of Losing Lead", x = 'Time Elapsed (%) When "2 Score" Lead Reached', color = NULL) + theme(legend.position="bottom") 
```