...
- Lots of new models in the literature, but they are often computationally intensive or use data that is hard to replicate in new locations.
...
## Findings: Three "Real World" Tests

**Predicting Future Flows**
- Random forest model (CPC: .83) substantially outperforms the best gravity model (.60).

**Generating Flows for an Additional City**
- In all but one simulation (Madrid) the random forest model is best.

**"Purchasing" a Subset of the Data & Generating the Rest**
- As the subset increases in size, the advantage of the random forest model grows.
...
```
Observed and generated flows for Alicant using data from the 38 other cities.
...
**Data Sources**
- INE Mobility: https://ine.es/experimental/movilidad/experimental_em.htm
